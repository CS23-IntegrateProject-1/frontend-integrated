...
export const Reservation = () => {
  ...
  const fetchBusinessInsightData = async () => {
    ...
    const [transactionResponse, transactionDetailResponse] = await Promise.all([
      ...
      Axios.get<transaction_detail[]>(`/feature8/venue/${venueId}/getReservationForAdmin`),
    ]);
...
const getNumberOfReservation = (groupedTransactionDetails: Record<string, transaction_detail[]>) => {
...
if(data){
  ...
  reservationNormal = getNumberOfReservation(groupedTDetailData).map(({ count }) => count);
}
...
const filteredReservation = getNumberOfReservation(groupedFilteredTDetailData).map(({ count }) => count);
...
console.log('reservationNormal:', reservationNormal);
    ...
    datasets: [
      {
        ...
        data: isFiltered ? filteredReservation : reservationNormal,
      ...
      >
        ...
        >
          ...
          <Popover colorScheme="#D9D9D9">
            <PopoverTrigger>
              <Link to={""}>
                <Text fontSize={"sm"}>Reservation</Text>
              </Link>
            </PopoverTrigger>
            <Portal>
              <PopoverContent>
        ...
        >
          <Box display={"flex"} justifyContent={"space-between"} ml={3}>
            <Text style={TextStyle.h3}>Number of Reservations</Text>
            <Text style={TextStyle.h3}>{isFiltered ?  
                    filteredReservation.reduce((total, count) => total + count, 0) : 
                    reservationNormal.reduce((total, count) => total + count, 0)
                  }</Text>
          </Box>
          <Bar data={reservation} options={chartOptions} />
        </Card>
        <Card
          marginTop={10}
          padding={5}
