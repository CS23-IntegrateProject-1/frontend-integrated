...
import { formatDate1 } from "../../../../functions/formatDatetime";
  ...
  const fetchBusinessInsightData = async () => {
    ...
    const [transactionResponse, transactionDetailResponse] = await Promise.all([
      ...
      Axios.get<transaction_detail[]>(`/feature8/getTransactionDetailForDelivery/${venueId}`),
    ]);
...
const getNumberOfDelivery = (groupedTransactionDetails: Record<string, transaction_detail[]>) => {
...
if(data){
  ...
  deliveryNormal = getNumberOfDelivery(groupedTDetailData).map(({ count }) => count);
}
      ...
      datasets: [
        {
          ...
          data: isFiltered ? filteredDelivery : deliveryNormal,
          ...
          >
            <Box display={"flex"} justifyContent={"space-between"} ml={3}>
              ...
              <Text style={TextStyle.h3}>{isFiltered ?  
                    filteredDelivery.reduce((total, count) => total + count, 0) : 
                    deliveryNormal.reduce((total, count) => total + count, 0)
                  }</Text>
            </Box>
