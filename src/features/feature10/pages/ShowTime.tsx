...
export const ShowTime = () => {
	...
	return (
		<>
			...
			{theatres
				.filter((theatre) =>
					theatre.Screens.some(
						(screen) =>
							screen.Shows.length > 0 &&
							screen.Shows.some((show) => {
								...
								console.log(show.filmId + " " + show.showId);
								console.log(
									parseInt(todayDate.slice(3, 5)) !=
										selectedDate ||
									...
									<Box key={screenIndex} mb={4}>
										...
										<Flex overflowX="auto">
											{screen.Shows.filter((show) => {
												...
												return (
													showDate > todayDate || showStartTime >= currentTime
												);
											}).map((show, showIndex) => (
												<Button
													key={showIndex}
													...
													onClick={() =>
														navigate(
															`/screen/${theatre.theaterId}/${movie.id}/${show.showId}`
														)
													}>
													{new Date(
														show.start_time
